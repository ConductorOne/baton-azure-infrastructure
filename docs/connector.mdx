...
| Role assignments | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> |

<Note>
**Role assignments** sync is gated behind the **Sync role assignments** toggle (cloud-hosted) or `BATON_SYNC_ROLE_ASSIGNMENTS` (self-hosted). It is **off by default**. When enabled, the connector emits each Azure RBAC role assignment as a scope-binding resource (with `TRAIT_SCOPE_BINDING`), which unlocks sparse-ACL / hybrid classification in the C1 uplift flow.
</Note>
...
- **When "Sync role assignments" is enabled (`--sync-role-assignments=true` / `BATON_SYNC_ROLE_ASSIGNMENTS=true`): Management Group Reader is required at the tenant root management group** (`/providers/Microsoft.Management/managementGroups/{tenantId}`). The connector fails fast at startup if the application lacks this role, with an error pointing at the exact scope to grant. This permission is needed to build the mgmt-group → subscription hierarchy that the sparse-ACL tree-view UX walks.
...
<Steps>
  ...
  **Optional.** Check the box if you want to **Enable syncing external resources** (identities from a shared identity source).
  ...
  <Step>
  **Optional.** Check the box if you want to **Sync role assignments**. When enabled, each Azure RBAC role assignment is emitted as a scope-binding resource (`TRAIT_SCOPE_BINDING`), enabling sparse-ACL / hybrid app classification in C1. This is **off by default** because scope-binding emission significantly increases sync volume. Requires at least **Reader** on the targeted subscription(s) and/or **Management Group Reader** at the relevant management-group scope for full coverage; provisioning of role assignments additionally requires write access on `Microsoft.Authorization/roleAssignments` (e.g. **User Access Administrator**) at the target scope.
  </Step>
  <Step>
...
stringData:
  ...
  # Optional: include if you want to sync Azure role assignments as scope-binding
  # resources (emits TRAIT_SCOPE_BINDING, enabling sparse-ACL / hybrid classification
  # in C1 uplift). Off by default. For provisioning, the SP also needs write on
  # Microsoft.Authorization/roleAssignments at the target scope.
  BATON_SYNC_ROLE_ASSIGNMENTS: true
