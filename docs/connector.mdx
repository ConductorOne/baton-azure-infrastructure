...
| Management groups (opt-in) | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> |  |
| Role assignments (opt-in) | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> |

<Note>
**Role assignments** and **Management groups** are opt-in resource types. They render as unchecked checkboxes during connector configuration in C1; enable them per tenant to emit each Azure RBAC role assignment as a scope-binding resource (with `TRAIT_SCOPE_BINDING`), which unlocks sparse-ACL / hybrid classification in the C1 uplift flow. For standalone / local runs, pass `--sync-resource-types=role_assignment,management_group,...` to the CLI.
</Note>
...
- **When the opt-in types (Role assignments / Management groups) are enabled: Management Group Reader is required at the tenant root management group** (`/providers/Microsoft.Management/managementGroups/{tenantId}`). The connector surfaces an actionable error at the first sync of those types if the application lacks this role. This permission is needed to build the mgmt-group → subscription hierarchy that the sparse-ACL tree-view UX walks.
- To provision role assignments (`role_assignment` resource type): The application must have write access on `Microsoft.Authorization/roleAssignments` at the target scope — the built-in **User Access Administrator** role grants this, or a custom role including the `Microsoft.Authorization/roleAssignments/write` and `.../delete` actions.
</Warning>
...
<Steps>
  ...
  **Optional.** Check the box if you want to **Skip syncing storage containers**.
  ...
  <Step>
  **Optional.** In the resource-type selection list, choose which resource types to sync. Most types are enabled by default; **Role assignments** and **Management groups** are opt-in and unchecked by default. Enable them to emit Azure RBAC role assignments as scope-binding resources (`TRAIT_SCOPE_BINDING`), which unlocks sparse-ACL / hybrid app classification in C1. Enabling these types requires at least **Management Group Reader** at the tenant root management group (see permissions warning above); provisioning of role assignments additionally requires write access on `Microsoft.Authorization/roleAssignments` (e.g. **User Access Administrator**) at the target scope.
  </Step>
  <Step>
  **Optional — paired with another identity connector (e.g. Microsoft Entra).** If the same C1 app already receives users / groups / managed identities from another connector, deselect those resource types here to avoid duplicate emission. C1's implicit baton-ID matching resolves grants across connectors in the same app.
  </Step>
  <Step>
...
stringData:
  ...
  # Optional: restrict which resource types the connector syncs. When unset,
  # all default-enabled types sync; opt-in types (role_assignment,
  # management_group) are skipped unless listed. Use this to:
  #   - Enable the opt-in sparse-ACL types: add role_assignment, management_group
  #   - Pair with another identity connector (e.g. Microsoft Entra) in the same C1 app:
  #     omit user, group, managed_identity so those come from the paired connector
  # Example (sparse-ACL + paired-with-entra):
  #   BATON_SYNC_RESOURCE_TYPES: tenant,management_group,subscription,resource_group,role,role_assignment,enterprise_application,storage_account,container
  BATON_SYNC_RESOURCE_TYPES: tenant,management_group,subscription,resource_group,role,role_assignment,enterprise_application,storage_account,container
